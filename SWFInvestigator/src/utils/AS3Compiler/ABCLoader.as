package utils.AS3Compiler
{
		...
		public class ABCLoader {
			...
			private static var swf_sprite_end:Array =
				// the extra code tells the player to instance a class "test" as a Sprite.
			 ...
			 * 
			 */
			public static function wrapInSWF(bytes:Array, includeSprite:Boolean = false):ByteArray {
				...
				if (includeSprite) {
					for (i=0;i<swf_sprite_end.length;i++) {
						out.writeByte(swf_sprite_end[i]);
					}
				} else {
			 ...
			 * 
			 */
			public static function loadBytes(bytes:*, inplace:Boolean=false):Loader {
